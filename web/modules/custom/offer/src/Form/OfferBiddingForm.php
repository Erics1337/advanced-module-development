...
class OfferBiddingForm extends FormBase {
  ...
  public function buildForm(array $form, FormStateInterface $form_state, $offer = NULL) {
    ...
    /**
     * Determines the starting price for the offer based on the offer type.
     *
     * If the offer type is 'with_minimum', the starting price is set to the value of the 'field_price' field.
     * If the offer type is 'no_minimum', the starting price is set to '0'.
     */
  ...
  public function validateForm(array &$form, FormStateInterface $form_state) {
    ...
    if($minium_price >= $form_state->getValue('bid')) {
      $form_state->setErrorByName('bid', $this->t('Minimum bid needs to be @price', ['@price' => (@$minium_price + 1)]));
    }

    // Server side validation for numeric field
    if (!is_numeric($form_state->getValue('bid'))) {
      $form_state->setErrorByName('bid', $this->t('Bid input needs to be numeric!'));
    }
  }
